  ...
  while( !gotIp ) {
    ...
    if(dat_p==0) {
      ...
      if( dhcpState != DHCP_STATE_OK ) {
        if (GetTick() > (lastDhcpRequest + 10000L) ){
          lastDhcpRequest = GetTick();
	  if( --dhcpTries <= 0 ) 
		  return 0;		// Failed to allocate address
          // send dhcp
          dhcp_start( buf, mymac, myip, mynetmask,gwip, dnsip, dhcpsvrip );
