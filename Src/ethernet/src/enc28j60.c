...
{
	SPI_Config(ENCJ_SPI, SPI_BaudRatePrescaler_64/2);  // divide 2 !IMPORTANT
}
...
void ENC28J60_GPIO_Configuration(void)
{
  ...
  /* Enable CS clocks */
  RCC_AHB1PeriphClockCmd(encjCS_RCC, ENABLE);

  /* Configure GPIO PIN for Chip select */
  GPIO_InitStructure.GPIO_Pin = encjCS_Pin;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(encjCS_Port, &GPIO_InitStructure);
...
{
	return SPI_SendByte(ENCJ_SPI, dt);
}
